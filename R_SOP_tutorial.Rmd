---
title: "Using RStudio for Statistical Analysis of PCBs"
...
author: |
  ...
  Dr. Rachel F. Marek (Associate Research Scientist)
  
  Dr. Andres Martinez (Assocaite Research Engineer)
...
This method covers an overview of data formatting and organization; instructions for download and installation of R and RStudio; an overview of the RStudio environment; basic functions, terminology, and operations; example calculations for PCB data; and how to make graphs and figures.
...
my_data <- read.csv("Example Data/my_file.csv")
...
```{r data-frames-index}
# indexing is similar to matrices

# call entire data frame
my_data[,]
...
# call a specific column by name
my_data$materials
```
---
...
A QQ (quantile-quantile) plot is a graphical tool used to assess whether a dataset follows a particular theoretical distribution. In this case, we are checking if the data follows a normal distribution. It works by plotting the quantiles of the observed data against the quantiles expected under normality. If the data are approximately normal, the points will fall roughly along a straight diagonal line. Deviations from this line suggest departures from normality. 

The Shapiro-Wilk test complements this visual assessment by providing a formal statistical test of normality. It computes a test statistic that measures how well the observed data conform to a normal distribution, producing a p-value as its output. A p-value greater than 0.05 is typically interpreted as insufficient evidence to reject the null hypothesis that the data are normally distributed, suggesting the data are approximately normal. Together, the QQ plot and Shapiro-Wilk test are commonly used in tandem because the plot offers intuitive visual insight while the test provides an objective, quantifiable decision criterion.

```{r normality}
# The ggpubr library is an extension of the ggplot library and is considered a convenient interface for creating publication ready plots 

# install.packages("ggpubr")
library(ggpubr)

# Reading in data from a blanks LOQ file stored in the working directory
my_data <- read.csv('./Example Data/EX 1/blanks_LOQ_20220919.csv', header = T, 
                    encoding = 'UTC-8')
...
sch_data <- read.csv('./Example Data/EX 1/schoolData.csv', header = T, 
                    encoding = 'UTC-8')

# Data frame columns can be indexed using $ following the format dataframe$columnName.
# New columns can also be created using this syntax:

sch_data$sum <- rowSums(sch_data[ , c(7:length(sch_data))])

# Base R summary
summary(sch_data$sum)

# The dplyr library is a powerful library that allows users to easily manipulate matrices and data frames.
# For more information, run ?dplyr in the console.

# install.packages("dplyr")
library(dplyr)

# The %>% operator is known as the "pipe operator" and is used in R to pass the 
# output of one function as the first argument to the next function in the 
# sequence

# summarize() creates a new data frame. It returns one row for each combination 
# of grouping variables. 

total.summary <- sch_data %>%
  summarize(
...
# The dplyr library also makes it easy to group categories together. For example, you can group data from this dataset by school, sample type (air or emissions), materials, or room. The categories in this case are the columns.

# Summary grouped by sample type. 

type.sum <- sch_data %>%
  group_by(sampletype) %>%
  summarize(
    minimum    = min(sum, na.rm = TRUE),
    mean_val   = mean(sum, na.rm = TRUE),
    sd_val     = sd(sum, na.rm = TRUE),
    maximum    = max(sum, na.rm = TRUE),
    n          = n()
  )
...
# You can also group multiple categories at once.

combo.sum <- sch_data %>%
  group_by(sampletype, school, materials) %>%
  summarize(
    minimum    = min(sum, na.rm = TRUE),
    mean_val   = mean(sum, na.rm = TRUE),
    sd_val     = sd(sum, na.rm = TRUE),
...
# Sometimes, there might be NAs in the resulting summary. In this case, there isn't always enough samples from a category to calculate a standard deviation. Therefore, it will return an NA in its place.
...
# Built-in dataset comparing tooth growth in guinea pigs.
data("ToothGrowth")

# Two groups: vitamin C delivered via orange juice (OJ) vs ascorbic acid (VC)
oj <- ToothGrowth$len[ToothGrowth$supp == "OJ"]
vc <- ToothGrowth$len[ToothGrowth$supp == "VC"]

# Check normality for each group before testing
shapiro.test(oj)
shapiro.test(vc)

# Independent samples t-test
# H0: There is no difference in mean tooth length between groups
# H1: There is a difference in mean tooth length between groups
result <- t.test(oj, vc)
print(result)

# p-value = 0.06, therefore we fail to reject the null hypothesis.
...
This example shows how to perform ANOVA test in R. Caution: ensure that you are comfortable with setting up and performing an ANOVA test before using this code. The example here was only produced to demonstrate how the code functions. This is not a statistics tutorial.
...
# Three groups: control, treatment 1, treatment 2
data("PlantGrowth")
...
# One-way ANOVA
# H0: There is no difference in mean plant weight across groups
...
# Only interpreted if ANOVA p-value is significant
TukeyHSD(result)
...
In this example, we calculate the Limit of Quantitation (LOQ) using method blanks.There are different ways to calculate LOQ, but we will only focus on one method here. An LOQ is calculated for each congener, so the final product will be a table with one value for every congener.

Before calculating the LOQ, it is important to check the normality of your data. Environmental data generally follows a log normal distribution. 

Here, the LOQ is calculated as the upper limit of the 99% confidence interval of log 10-transformed blank masses.First, all congener masses are log 10-transformed. Next, the average and standard deviation are calculated for each congener. Then, the 99% confidence interval is calculate as: 

$$
\mu + 2.325 \left(\frac{\sigma}{\sqrt{n}}\right)
$$
where $\mu$ is the log-transformed congener mass, $\sigma$ is the log-transformed congener standard deviation, and n is the number of observations, or number of blanks. Finally, untransform the values to get your final LOQ as a mass for each congener.
...
# this typcially isn't an issue if you don't quantify any zeroes, but sometimes they still slip through
...
In this section, we calculate indoor air concentration using the sampling rate (R~s~) of a PUF-PAS and the effective volume (V~eff~). Data required to calculate air concentrations include: congener specific properties (see kpuf.csv and koaCalc.csv), congener specific mass, general room conditions, deployment time

> **Note:** This calculation is specifically for PUF-PAS deployed indoors. Outdoor air requires a different method for concentration determination.

## For Loops

Loops are an essential part of coding that is useful for a variety of tasks.They allow you to repeat a block of code a set number of times, iterating over a sequence or collection of values automatically. Rather than writing out the same operation manually for each item, loops move through each element one by one and applies the same logic to each.

For loops are a type of loop that iterates over a user-specified interval. Code inside the for loop will repeat until the end condition is met. 
...
# For loop that prints the square of each number
# i is the variable or value being iterated. Each time the loop restarts, i increases.

for (i in numbers) {
  
  squared <- i^2
  
  # cat() (concatenate) prints the combination of variables or text provided in the parentheses. 
  
  cat(i, "squared is", squared, "\n") 
...
In R, custom functions allow you to package a block of code into a reusable unit that can be called whenever needed. Rather than copying and pasting the same lines of code multiple times throughout your script, you define a function once and simply call it by name. 

A function takes in inputs called arguments, performs some operations on them, and returns an output. For example, you might write a function that accepts  variables and runs calculations. That entire workflow can be executed with a single line of code rather than rewriting it each time. This becomes especially useful as your scripts grow more complex, keeping your code organized, readable, and less prone to errors that can creep in when duplicating code manually.

This example shows how to structure a custom function and how it can be used to calculate air concentrations.

```{r air-functions}

# To calculate PCB air concentrations from a PUF-PAS, first, the k_puf must be calculated.
...
# Here, a new function is defined that can be called and executed in a later part of a script 

conc.calc <- function(fsch.air,veff,flist){
  # Begins by creating a data frame named "sch.conc" with an equal number of rows and columns as fsch.air data frame
  sch.conc <- data.frame(matrix(nrow = nrow(fsch.air), ncol = ncol(fsch.air))) 
  for (c.i in 1:nrow(fsch.air)) {
    for (c.j in  1:ncol(fsch.air)) {
      sch.conc[c.i,c.j] <- fsch.air[c.i,c.j] / veff[c.j,2]  # References row i and column j from the fsch.air data frame
    }
  }
  sch.conc.t <- as.data.frame(rowSums(sch.conc)) # Sums the rows of sch.conc 
  sch.conc.t <- signif(sch.conc.t,2)        # significant figures
  names(sch.conc.t) <- 'Concentration'     # Sets column name to 'Concentration' 
...
Import data provided from the "Ex 3" folder. This includes PCB congener properties, sample mass from Hua et al. 2023 (https://doi.org/10.1021/acs.est.3c05195), and sample type information.

Parameters used in this model are from Herkert et al. 2018 (https://doi.org/10.1039/C7EM00360A) and Bannavti and Jahnke et al. 2021 (https://doi.org/10.1021/acs.est.0c08149). 
...
# Takes the first row, removes first two columns, transposing the remaining 
# values into a column, and converting to a data frame
...
Here, we initialize our variables based on values from Herkert et al. 2018 and Bannavti and Jahnke et al. 2021. These can differ depending on the nature of the project. 

```{r air-params}

t.days <- 29    #deployment time, days
ws.on <- 0.11   #indoor wind speed when HVAC is on, m/s
ws.off <- 0.07  #indoor wind speed when HVAC is off, m/s
f.on <- 0.36    #fraction HVAC is on, unitless
f.off <- 1 - f.on #fraction HVAC is off, unitless
c <- 1.326      #sampler constant, unitless
puf.sa <- 0.0153 #PUF surface area, m2
v.puf <- 2.295e-04 #PUF volume, m3 
den.puf <- 21300 #PUF density, g/m3 
temp.c = 23      # temperature in C (73 F used)
...
# Calls a previously defined function that is to use pcb, R, and temp.c values
...
# Creates a data frame with an equal number of rows as the pcb data frame with 
# 1 column
...
# Remove specific congeners from data frame
...
# Combine the congener.list and V.eff data frame into a single data frame 
...
One of R's biggest strengths is its ability to create and customize plots. This example will walk through different ways to create plots and figures. Data used in this section is found in the folder "Ex 4". There are also many cheat sheets and guides available online describing how to use plotting functions. Thess are found in the "Cheat Sheets" folder.
...
plot(x, y,
     main = "My Plot Title",  # Plot title
     xlab = "X Axis Label",   # X axis title
     ylab = "Y Axis Label",   # Y axis title
...
One of the most common libraries used in R is ggplot2. This library is a powerful tool for customizing plots and figures. There is also a cheat sheet for using ggplot2 available in the "Cheat Sheets" folder.
...
This example introduces how to use ggplot2 with a boxplot example.

```{r ggplot-boxexample}
library(ggplot2)

# all school concentration stats

all.conc <- read.csv('./Example Data/Ex 4/allSchoolConc.csv', header = T, fileEncoding = 'UTF-8-BOM', check.names = F)

# subsets data into two new data frames, one with Vermont concentration data and one with UIowa data

vt.samp <- all.conc[all.conc$Sampler=='vt',c(1,3)]
ui.samp <- all.conc[all.conc$Sampler=='ui',c(1,3)]

sch.list <- distinct(ui.samp, school)

low.sch <- c('C P Smith School', 'Cabot School', 'Cavendish Town Elem School',
             'Charlotte Central School', 'Danville School', 'Essential Early Education',
             'J J Flynn School', 'North Hero School', 'Twinfield US #33')
high.sch <- c('Bellows Falls UHS #27', 'Brighton Elementary School', 'Green Mountain UHS #35',
              'North Country UHS #22A', 'Oak Grove School', 'Soar Learning Center', 'Twin Valley Elementary School')

# check: filtered not found
ggplot(filtered, aes(x = materials, y = em, fill = school_group)) +
...
ggplot(filtered, aes(x = materials, y = em, fill = school_group)) +
  ...
  theme_classic() +
  labs(y = expression(paste('Emissions (ng m'^{-2}*' d'^{-1}*')'))) +
  theme(
...
ggplot(filtered, aes(x = materials, y = em, fill = school_group)) +
  ...
  scale_y_log10(limits = c(10,100000), labels = scales::comma) +
  scale_fill_manual(values = c("Low" = "#1f77b4", "High" = "#ff7f0e")) +
  scale_fill_manual(
    name = "",
    values = c("Low" = "#1f77b4", "High" = "#ff7f0e"),
    labels = c("Low" = "Schools Below SAL", "High" = "Schools Above SAL")
  ) 

```

## Congener Profile Plots 

This section covers how to create a simple congener distribution or congener profile plot. This plot shows the relative abundance of each congener by mass. Note that in some cases, calculating the concentration or emission fraction is preferable. 

First, calculate the mass fraction of every congener for each sample: 

$$mass \ fraction = mass_{PCBi} \div mass_{total}$$

where `mass~PCBi~` is the mass of an individual congener, i, and `mass~total~` is the total mass of the sample. When the mass fraction is summed for all congeners in a sample, the total should be 1. Mass fraction can, and often is, expressed as a percentage.
...
mass <- read.csv('./Example Data/Ex 4/sampleMass.csv', header = T, fileEncoding = 'UTF-8-BOM', check.names = F)
cong.list <- read.csv('./Example Data/Ex 4/sampleMass.csv', header = F, fileEncoding = 'UTF-8-BOM', check.names = F)

#extract congener list

cong.list <- cong.list[1,-c(1,2)]
cong.list <- as.data.frame(t(cong.list))

# clean data frames so they can be used for calculations

bid <- mass[,1]       # extract batch id list for later
mass.1 <- mass[,-c(1,2)]
mf <- mass.fraction(mass.1,cong.list)   # runs mass fraction calculation 
mf <- mf * 100                # multiplied by 100 to present as a percentage in the figure
mf.t <- as.data.frame(t(mf))
mf.t <- cbind(cong.list,mf.t)

mf <- cbind(bid,mf)

```

After the mass fraction is calculated, use the code below to plot the congener profile. Multiple profile plots are typically created at a time, therefore, it is more efficient to create a function that will create many plots simultaneously. The example below is a function that loops through a data frame of 
...
prof.plot <- function(data, title){
  
 data[,1] <- factor(data[,1], level = data[,1])    # this keeps the plot from changing the congener order
