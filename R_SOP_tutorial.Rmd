...
[DMAC data management info here]

## File and Metadata Naming Conventions
...
- Samples in rows, data/categories in columns
- Reducing redundancy
...
**RStudio** is an integrated development environment (IDE) for R — software that streamlines user experience by providing tools such as a user interface, code editor, and debugger.
...
A **vector** is one of the most common data structures in R — a series of values of the **same type**, created with the `c()` function.
...
**R Markdown** is a file format that combines plain text, R code, and output (plots, tables, results) into a single reproducible document. Files are saved with the `.Rmd` extension and can be rendered ("knit") to HTML, PDF, or Word.

**Key components of an R Markdown document:**

- **YAML header** — at the top (between `---`), controls title, author, date, and output format
- **Narrative text** — written in Markdown for headers, bold, lists, links, etc.
- **Code chunks** — enclosed in ` ```{r} ` ... ` ``` ` blocks; contain executable R code

**To knit this document:**
